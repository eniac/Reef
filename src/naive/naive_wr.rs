...
pub fn naive_bench(
    ...
    if doc.len() % batch_size != 0 {
        let padding = (0..(doc.len() % batch_size) + 1)
            .map(|_| '\0')
            .collect::<String>();
    ...
    let mut prover_states: Vec<u32> = solution
        .clone()
        .into_iter()
        .map(|(a, b, c)| a)
        .collect_vec();
    ...
    let pc: PoseidonConstants<<G1 as Group>::Scalar, typenum::U4> =
    ...
    for i in 0..(doc_len / batch_size) {
        ...
        private_input.insert(
            "states".to_string(),
            json!(prover_states[(batch_size * i)..(batch_size * (i + 1) + 1)]),
        );
        private_input.insert(
            "chars".to_string(),
            json!(doc_vec[(batch_size * i)..(batch_size * (i + 1))]),
        );
