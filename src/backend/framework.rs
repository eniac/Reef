...
struct Witness<'a, F: PrimeField> {
    pub r1cs: &'a R1csFinal, // TODO later ref
    pub values: Option<Vec<Value>>,
    pub states: Vec<F>,
    pub glue: Vec<GlueOpts<F>>,
    pub commit_blind: F,
    pub first: bool,
    pub epsilon_num: F,
    pub start_of_ep: isize,
    pub accepting: Vec<F>,
    pub batch_size: usize,
    pub pcs: PoseidonConstants<F, typenum::U4>,
}

struct SetupParams<'a> {
    pp: PublicParams<G1, G2, C1<'a>, C2>,
...
impl<'a> Backend {
    ...
    pub fn run_backend(
        ...
        let (sender, recv): (
            Sender<Witness<'a, <G1 as Group>::Scalar>>,
            Receiver<Witness<'a, <G1 as Group>::Scalar>>,
        ) = mpsc::channel();

        let (send_setup, recv_setup): (Sender<SetupParams>, Receiver<SetupParams>) =
            mpsc::channel();
        ...
        let solver_thread = thread::spawn(move || {
            ...
            let (num_steps, z0_primary, z0_secondary, pp) =
                Self::setup(&r1cs_converter, &prover_data); //, timer);
            send_setup
                .send(SetupParams {
                    pp,
                    z0_primary,
            ...
            Self::solve(sender, &r1cs_converter, &prover_data, num_steps); //, timer);
    ...
    fn setup(
        ...
        */
        ...
        (num_steps, z0_primary, z0_secondary, pp)
    }

    fn solve(
        sender: Sender<Witness<'a, <G1 as Group>::Scalar>>,
        r1cs_converter: &R1CS<<G1 as Group>::Scalar>,
        circ_data: &'a ProverData,
        num_steps: usize,
        //    timer: &mut Timer,
        ...
        for i in 0..num_steps {
            ...
            let witness_i = Witness {
                ...
                commit_blind: blind,
                first: i == 0,
                epsilon_num: <G1 as Group>::Scalar::from(r1cs_converter.dfa.nchars() as u64),
                start_of_ep: start_of_epsilons,
                accepting: vec![
                    <G1 as Group>::Scalar::from(
                        r1cs_converter.prover_accepting_state(i, current_state),
                    ),
                    <G1 as Group>::Scalar::from(
                        r1cs_converter.prover_accepting_state(i, next_state),
                    ),
                ],
                batch_size: r1cs_converter.batch_size,
                pcs: r1cs_converter.pc.clone(),
            };

            sender.send(witness_i).unwrap();
    ...
    fn prove(
        recv: Receiver<Witness<'a, <G1 as Group>::Scalar>>,
