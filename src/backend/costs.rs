...
fn accepting_circuit<'a>(dfa: &'a DFA, is_match: Option<(usize,usize)>) -> usize {
...
fn commit_circuit_nohash(doc_len: usize, batch_size: usize, commit_type: JCommit,is_match: Option<(usize,usize)>) -> usize {
    match commit_type {
...
fn commit_circuit_hash(doc_len: usize, batch_size: usize, commit_type: JCommit,is_match: Option<(usize,usize)>) -> usize {
    match commit_type {
        JCommit::HashChain => {
            match is_match {
...
pub fn opt_cost_model_select<'a>(
    ...
    for n in batch_range_lower..batch_range_upper + 1 {
        let batching_and_cost: (JBatching, JCommit, usize) = match (batching.clone(),commit.clone(),can_hashcahin) {
           (None,None,_) => opt_cost_model_select_with_batch(dfa, 2 << n, is_match, doc_length),
           (_,Some(JCommit::HashChain),false) => (JBatching::NaivePolys,JCommit::HashChain,cost+100),
           (None,Some(c),_) => opt_cost_model_select_with_commit(dfa, 2<<n, is_match, doc_length, c),
           (Some(b),None,_) => opt_commit_select_with_batch(dfa, 2<<n, is_match, doc_length, b),
           (Some(b),Some(c),_)=> (b,c,full_round_cost_model(dfa, 2<<n, b, is_match, doc_length, c)),
        };  
