...
pub fn nl_nohash<'a>(batch_size: usize, table_size: usize) -> usize {
...
pub fn nlookup_cost_hash(batch_size: usize, table_size: usize, hybrid: bool) -> usize {
    let log_mn: usize = logmn(table_size);
    let num_cqs = ((batch_size * log_mn) as f64 / 254.0).ceil() as usize;
    let mut cost = 0;
...
pub fn get_folded_cost(cost: usize, solution_lens: Vec<usize>, batch_size: usize) -> usize {
    ...
    let n_foldings: Vec<usize> = solution_lens
        .iter()
        .map(|x| ((*x as f32) / batch_f32).ceil() as usize)
        .collect();
...
pub fn opt_cost_model_select_with_batch<'a>(
    ...
    (batch_size, get_folded_cost(cost, solution, batch_size))
}
