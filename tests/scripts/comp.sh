...
echo "reef"
for i in {1..20}; do ./tests/scripts/reef.sh &> out_reef; done
echo "end reef"

echo "reef h"
for i in {1..20}; do ./tests/scripts/reef_h.sh &> out_rh; done
echo "reef h"
...
for i in {1..20}; do ./tests/scripts/nwr.sh &> out_nwr; done 
echo "nwr"

cargo build --release --features 'metrics,naive'
echo "naive"
for i in {1..20}; do ./tests/scripts/naive.sh &> out_naive; done 
