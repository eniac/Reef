...
impl Timer {
    pub fn new() -> Timer {
        Timer { log : DashMap::new() }
    }

    pub fn tic(&mut self, comp: Component, test: &str, subtest: &str) {
        if self.log.contains_key(&(comp.clone(), test.to_string(), subtest.to_string())) {
            self.log.alter(&(comp, test.to_string(), subtest.to_string()), |_, v|
                match v {
            ...
            self.log.insert((comp, test.to_string(), subtest.to_string()),
                        Started(Instant::now()));
        }
    }

    pub fn stop(&mut self) {
        self.log.alter_all(|_, v|
            match v {
                Started(start_time) => Finished(start_time.elapsed()),
