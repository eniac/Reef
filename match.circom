pragma circom 2.0.3;
    ...
    template IsValidTrans() {
        ...
        component runningProduct = MultiplierN(129664);

        for (var i = 0; i < 129664; i++) {
    ...
    template Main () {
        signal input doc[1000];
        signal input prover_states[1001];
        signal input blind;
        ...
        for (var j=1;j<1001;j++) {
            ...
            valid_state[j-1].curIndex <== prover_states[j-1]*1013*128 + doc[j-1]*1013 +
            prover_states[j];
            valid_state[j-1].out === 0;
        }
        valid_match = IsValidMatch();
        valid_match.in <== prover_states[1000];
    
        valid_match.out === 0;

        component hash = PoseidonMulti(1000);
        hash.in <== doc;
        hash.blind <== blind;
